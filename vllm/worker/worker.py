...
class MyWorkerConfig(object):
    ...
    def __init__(self):
        super(MyWorkerConfig, self).__init__()
        import os
...
class Worker:
    ...
    def __init__(
        ...
        self.my_worker_config = MyWorkerConfig()
    ...
    # <jingzhi> this is the original function of vllm, which does not support page-recompute
    def _prepare_inputs_vllm(
        self,
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            for i in range(prompt_len):
                ...
                # <jingzhi> DEBUG
                # if (seq_id == 157) and (i in [0, 200, 1019]):
                #     layer_i = 2
                #     print(f"seq_id: {seq_id}, layer_i: {layer_i}, token_i: {i}, prompt")
                #     # [num_blocks, num_kv_heads, head_size/x, block_size, x]
                #     print(self.gpu_cache[layer_i][0][block_number][0])
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            for seq_id in seq_ids:
                ...
                #     layer_i = 2
                #     for i in [0, 200, 1019]:
                #         print(f"seq_id: {seq_id}, layer_i: {layer_i}, token_i: {i}, generate")
                #         # [num_blocks, num_kv_heads, head_size/x, block_size, x]
                #         print(self.gpu_cache[layer_i][0][block_table[i // self.block_size]][0])
    ...
    # <jingzhi> support page-recompute [we prepare inputs for page-recompute here]
    def _prepare_inputs(
        self,
        seq_group_metadata_list: List[SequenceGroupMetadata],
    ) -> Tuple[torch.Tensor, torch.Tensor, InputMetadata]:
        ...
        # Add generation tokens. (also deal with the page-recompute cases)
        # For page-recompute: we treat each token to be recomputed as a unique sequence ending token
        # and prepare inputs for them
        max_context_len = 0
        max_num_blocks_per_seq = 0
        context_lens: List[int] = []
        generation_block_tables: List[List[int]] = []

        # <jingzhi> used in sampling to get the tokens that we will use to generate next tokens
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            for seq_id in seq_ids:
                ...
                # <jingzhi>
                compute_poses = None
                if seq_group_metadata.to_recompute_blocks:
                    # recompute_poses do not include the last token because anyway it does not have kv cache
                    compute_poses = np.concatenate(\
                        [range(i*self.block_size, min((i+1)*self.block_size, context_len - 1)) \
                            for i in seq_group_metadata.to_recompute_blocks] + [[position]])
                    compute_tokens = np.asarray(seq_data.get_token_ids())[compute_poses]
                    input_tokens.extend(compute_tokens.reshape((-1, 1)).tolist())
                    input_positions.extend(compute_poses.reshape((-1, 1)).tolist())

                    # get generation block tables and content lengths
                    tmp_block_tables = [block_table] * len(compute_poses)
                    tmp_context_lens = compute_poses + 1
                    if self.sliding_window is not None:
                        sliding_window_blocks = (self.sliding_window //
                                                 self.block_size)
                        # the block window should include the one containing the target token
                        tmp_block_tables = [block_table[\
                                            i//self.block_size+1-sliding_window_blocks:\
                                            i//self.block_size+1] for i in compute_poses]

                        tmp_context_lens = np.minimum(tmp_context_lens, self.sliding_window)
                    
                    generation_block_tables.extend(tmp_block_tables)
                    context_lens.extend(list(tmp_context_lens))

                else:
                    compute_poses = np.asarray([position])
                    input_tokens.append([generation_token])
                    input_positions.append([position])
                ...
                # <jingzhi> used in sampling-----------------------
                selected_token_indices.append(len(input_tokens)-1)

                # Compute the slot mapping.
                block_numbers = np.asarray([block_table[i]for i in (compute_poses // self.block_size)])
                block_offsets = compute_poses % self.block_size
                slots = block_numbers * self.block_size + block_offsets
                slot_mapping.extend(slots.reshape((-1, 1)).tolist())
    ...
    def execute_model(
        ...
        if blocks_to_swap_in:
            ...
            # <jingzhi> Try page-swap and page-recompute
            if not self.my_worker_config.use_our_method:
