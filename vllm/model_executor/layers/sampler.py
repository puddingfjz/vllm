...
# <jingzhi> support page-recompute
def _prune_hidden_states(
    hidden_states: torch.Tensor,
    input_metadata: InputMetadata,
) -> torch.Tensor:
    selected_token_indices: List[int] = []
    if len(input_metadata.selected_token_indices) > 0:
        # this iteration is for decoding, and there may be page-recompute
        selected_token_indices = torch.tensor(input_metadata.selected_token_indices,
                                              dtype=torch.long,
                                              device=hidden_states.device)
        hidden_states = hidden_states.view(-1, hidden_states.shape[-1])
        return hidden_states.index_select(0, selected_token_indices)

    # normal process in vllm
    start_idx = 0
    for i, seq_group in enumerate(input_metadata.seq_groups):
        seq_ids, sampling_params = seq_group
        if i < input_metadata.num_prompts:
