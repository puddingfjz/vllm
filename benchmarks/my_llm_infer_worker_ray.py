...
@ray.remote
class REMOTE_LLM_INFER_WORKER_MESSAGE_PASSER:
    """A ray actor class used to notify the ray workers when to stop inference.
    """
    def __init__(self):
...
class REMOTE_LLM_INFER_WORKER:
    ...
    def __init__(self,
        ...
        message_passer, # a ray actor used to stop data parallel ray actors
        cuda_visible_devices: str, 
        communicator: LLM_COMMUNICATOR,
        ):

        print(f"\ninitializing a ray worker!\n")

        # # set model_i for this actor
        # SHARED_CONTECT.init_SHARED_CONTECT_by_copy(worker_i, SHARED_CONTECT_to_copy)
        SHARED_CONTECT.shared_id = model_i
        SHARED_CONTECT.communicator = communicator

        os.environ["CUDA_VISIBLE_DEVICES"] = cuda_visible_devices

        self.worker = LLM_INFER_WORKER(worker_i, remaining_requests, message_passer)
    # 
...
class LLM_INFER_WORKER:
    """A worker class that executes (a partition of) the request on GPU(s).

    Each worker is associated with an LLM engine.
    """
    ...
    def __init__(
        ...
        # 明日再战！要注意在model init里设置ignore_reinit_error=True
        # 是否需要在这里设置一些shared_mem里面的process setting？
    ...
    def do_inference(
        ...
        gpus = gpus[self.worker_i*tensor_parallel_size:] + gpus[:self.worker_i*tensor_parallel_size]
        ...
        if (self.rescheduled_iter_num == 0) or (os.environ['SOFT_RESCHEDULE'] == 'False'):
        ...
        if (len(self.remaining_requests)>0) and isinstance(self.remaining_requests[0], tuple):
            ...
            for prompt, _, output_len in self.remaining_requests:
            ...
            llm.llm_engine.scheduler.waiting = self.remaining_requests
        ...
        print(f"worker i: {self.worker_i}, len of outputs and remainings before return: {len(SHARED_CONTECT.gened_outputs), len(SHARED_CONTECT.remaining_requests)}")
